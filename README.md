...
- `PHP 5.6` and newer PHP versions are supported on self-hosted runners.
- `Windows 7` and newer, `Windows Server 2012 R2` and newer, `Ubuntu 18.04`, `Ubuntu 16.04` and `MacOS X Catalina 10.15` are supported.
- To setup a self-hosted runner on `ubuntu` in a docker container, refer to this [Dockerfile](https://github.com/shivammathur/setup-php/wiki/Dockerised-self-hosted-runner-on-ubuntu). 
- To setup without docker, follow this [requirements guide](https://github.com/shivammathur/setup-php/wiki/Requirements-for-self-hosted-runners "Requirements guide for self-hosted runner to run setup-php") before setting up the self-hosted runner.
...
Run the workflow locally using `act` with [`shivammathur/node`](https://github.com/shivammathur/node-docker "Docker image to run setup-php") docker image.
